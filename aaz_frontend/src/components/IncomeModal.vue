    ...
    <v-card class="modal-card">
      ...
      <v-card-text class="scrollable-content pt-6">
        <v-form ref="form" v-model="valid">
          <!-- 日期 -->
          <v-text-field v-model="formData.incomeDate" label="日期" type="date" prepend-icon="mdi-calendar"
              ...
              <v-list density="compact">
                <v-list-item v-for="member in members" :key="member.id">
                  <template v-slot:prepend>
                    <v-checkbox v-model="selectedReceivers" :value="member.id" hide-details density="compact"
                      :disabled="isViewMode"></v-checkbox>
                  </template>
                  <v-list-item-title>{{ member.displayName }}</v-list-item-title>
                  <template v-slot:append>
                    <v-text-field v-if="selectedReceivers.includes(member.id)" v-model="receiveAmounts[member.id]"
...
// 監聽收款人變化，自動平均分配金額
watch(selectedReceivers, (newReceivers) => {
  if (newReceivers.length > 0 && formData.value.amount) {
    const perPerson = parseFloat(formData.value.amount) / newReceivers.length;
    newReceivers.forEach((id) => {
