<template>
  <v-dialog v-model="dialog" max-width="600px" persistent>
    <v-card>
      <v-card-title class="text-h5 bg-red-lighten-4">
...
const emit = defineEmits(['update:modelValue', 'expense-added']);
...
const submitExpense = async () => {
  ...
  try {
    ...
    await api.post(`/trips/${props.tripId}/expenses`, payload);
    emit('expense-added');
    closeDialog();
  } catch (error) {
